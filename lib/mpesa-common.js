...
const setupMpesa = async(req) => {
    const config = req.app.config;
    const paymentConfig = getPaymentConfig('mpesa');
    ...
    const payload = {
        ...
        amount: numeral(req.session.totalCartAmount).format('0.0000').replace('.', ''),
        configurations: {
            card: {
        ...
        return_url: `${config.baseUrl}/mpesa/checkout_return`
    };
